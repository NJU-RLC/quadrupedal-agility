...
class Logger:
    ...
    def plot_dof_pos(self):
        self.plot_process = Process(target=self._plot_dof_pos)
        self.plot_process.start()
    ...
    def _plot(self):
        ...
        a = axs[1, 0]
        if log["dof_pos"]: a.plot(time, np.vstack(log["dof_pos"])[:, joint_index], label='measured')
        ...
        # plot joint velocity
        a = axs[1, 1]
        if log["dof_vel"]: a.plot(time, np.vstack(log["dof_vel"])[:, joint_index], label='measured')
        ...
        a = axs[2, 1]
        if log["dof_vel"] != [] and log["dof_torque"] != []: a.plot(np.vstack(log["dof_vel"])[:, joint_index],
                                                                    np.vstack(log["dof_torque"])[:, joint_index], 'x',
                                                                    label='measured')
    ...
    def _plot_dof_pos(self):
        ...
        for i in range(nb_rows):
            for j in range(nb_cols):
                a = axs[i, j]
                if log["dof_pos"]: a.plot(time, np.vstack(log["dof_pos"])[:, idx], label='measured')
                if log["dof_pos_target"]: a.plot(time, np.vstack(log["dof_pos_target"])[:, idx], label='target')
                a.set(xlabel='time [s]', ylabel='Position [rad]', title='DOF Position')
                a.legend()
                idx += 1
        plt.show()
