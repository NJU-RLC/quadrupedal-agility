...
class Discriminator(nn.Module):
    def __init__(self, env, input_dim, num_disc_obs, dim_c, dt, disc_loss_function, reward_i_normalizer, reward_i_coef,
                 reward_us_coef, reward_ss_coef, reward_t_coef, disc_history_len, disc_obs_len, obs_disc_weight_step,
                 hidden_units, device):
    ...
    def predict_disc_reward(self, reward_t, obs, obs_disc, normalizer=None):
        ...
        obs_disc = obs_disc.clone()
        if self.env.task_obs_weight_decay:
            obs_disc[:, :, 3:9] *= self.env.task_obs_weight  # lin_vel, ang_vel
            obs_disc[:, :, 33:] *= self.env.task_obs_weight  # key_pos, foot_contact
        ...
        multipliers = multipliers.view(1, -1, 1).repeat(obs_disc.size(0), 1, self.num_disc_obs).view(len(obs_disc), -1)
        obs_disc = obs_disc * multipliers

        with (torch.no_grad()):
            self.eval()
            if normalizer is not None:
                obs_disc = normalizer.normalize_torch(obs_disc, self.device)
