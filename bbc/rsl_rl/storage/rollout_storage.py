...
class RolloutStorage:
    class Transition:
        def __init__(self, num_envs, obs_shape, privileged_obs_shape, actions_shape, device='cpu'):
            self.observations = torch.zeros(num_envs, *obs_shape, device=device)
            self.critic_observations = torch.zeros(num_envs, *privileged_obs_shape, device=device)
            self.actions = torch.zeros(num_envs, *actions_shape, device=device)
            self.rewards = torch.zeros(num_envs, device=device)
            self.dones = torch.zeros(num_envs, device=device).byte()
            self.values = torch.zeros(num_envs, 1, device=device)
            self.actions_log_prob = torch.zeros(num_envs, device=device)
            self.action_mean = torch.zeros(num_envs, *actions_shape, device=device)
            self.action_sigma = torch.zeros(num_envs, *actions_shape, device=device)
    ...
    def __init__(self, num_envs, num_transitions_per_env, obs_shape, privileged_obs_shape, actions_shape,
        ...
        self.sigma = torch.zeros(num_transitions_per_env, num_envs, *actions_shape,
                                 device=self.device)
