...
import torch
import time
from legged_gym.utils.torch_jit_utils import quaternion2rpy
from legged_gym.utils.math import trans_matrix
...
class BaseTask():
    ...
    def __init__(self, cfg, sim_params, physics_engine, sim_device, headless):
        ...
        self.num_obs_bbc = cfg.env.num_observations_bbc
        self.num_privileged_obs = cfg.env.num_privileged_obs
        self.num_actions_d = cfg.env.num_actions_d
        self.num_actions_c = cfg.env.num_actions_c
        self.num_actions = cfg.env.num_actions_bbc
        ...
        self.obs_bbc_buf = torch.zeros(self.num_envs, self.num_obs_bbc, device=self.device, dtype=torch.float)
    ...
    def get_observations_bbc(self):
        return self.obs_bbc_buf
    ...
    def reset(self):
        ...
        obs, privileged_obs, _, _, _ = self.step(torch.zeros(self.num_envs, 1 + self.num_actions_d *
                                                             self.num_actions_c, device=self.device,
                                                             requires_grad=False))
    ...
    def lookat(self, i):
        camera_pos_bias = np.array([-1.8, -0.8, 0.8])
        robot_pos = self.root_states[i, :3].cuda().cpu().numpy()
        robot_quat = self.root_states[i, 3:7].cuda().cpu().numpy()
        robot_ang = np.array(quaternion2rpy(robot_quat))
        camera_position = (trans_matrix(robot_pos, [0, 0, robot_ang[-1]]) @ np.append(camera_pos_bias, 1))[:-1]
        # camera_position = (trans_matrix(robot_pos, [0, 0, 0]) @ np.append(camera_pos_bias, 1))[:-1]
        self.set_camera(camera_position, robot_pos)
