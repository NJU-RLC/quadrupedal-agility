...
class RolloutStorage:
    class Transition:
        def __init__(self):
            ...
            self.actions_log_prob_d = None
            self.actions_log_prob_c = None
    ...
    def __init__(self, num_envs, num_transitions_per_env, obs_shape, privileged_obs_shape, actions_shape, device='cpu'):
        ...
        # For PPO
        self.actions_log_prob_d = torch.zeros(num_transitions_per_env, num_envs, 1, device=self.device)
        self.actions_log_prob_c = torch.zeros(num_transitions_per_env, num_envs, 1, device=self.device)
        self.values = torch.zeros(num_transitions_per_env, num_envs, 1, device=self.device)
        self.returns = torch.zeros(num_transitions_per_env, num_envs, 1, device=self.device)
        self.advantages = torch.zeros(num_transitions_per_env, num_envs, 1, device=self.device)
        self.mu = torch.zeros(num_transitions_per_env, num_envs, actions_shape[0] - 1, device=self.device)
        self.sigma = torch.zeros(num_transitions_per_env, num_envs, actions_shape[0] - 1, device=self.device)
    ...
    def add_transitions(self, transition: Transition):
        ...
        self.actions_log_prob_d[self.step].copy_(transition.actions_log_prob_d.view(-1, 1))
        self.actions_log_prob_c[self.step].copy_(transition.actions_log_prob_c.view(-1, 1))
    ...
    def mini_batch_generator(self, num_mini_batches, num_epochs=8):
        ...
        old_actions_log_prob_d = self.actions_log_prob_d.flatten(0, 1)
        old_actions_log_prob_c = self.actions_log_prob_c.flatten(0, 1)
        ...
        for epoch in range(num_epochs):
            for i in range(num_mini_batches):
                ...
                old_actions_log_prob_d_batch = old_actions_log_prob_d[batch_idx]
                old_actions_log_prob_c_batch = old_actions_log_prob_c[batch_idx]
